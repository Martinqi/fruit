...
class content extends main {
    ...
    function addCon(){
        ...
        $con2=$_POST['con2'];
        $con3=$_POST['con3'];
        $imgurl=$_REQUEST["simage"];
        $pe=$_POST["posid"];
        $posid=implode(";",$_POST["posid"]);

        $db=new db("content");
        $result=$db->insert("title='{$title}',jiacu='{$jiacu}',Engtitle='{$Engtitle}',price='{$price}',kucun='{$kucun}',keywords='{$keywords}',con='{$con}',con2='{$con2}',con3='{$con3}',posid='{$posid}',cid='{$cid}',thumb='{$imgurl}'");
        if($result>0){
